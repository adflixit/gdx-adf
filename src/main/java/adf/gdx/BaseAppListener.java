...
import adf.gdx.TweenUtil.SpriteAccessor;
 ...
 * <ul>
 * <li>{@link Console}.</li>
 * <li>{@link Skin} handlers.</li>
 * <li>{@link MutableProperties}.</li>
 * <li>Preferences handlers.</li>
 * </ul>
 */
public abstract class BaseAppListener implements ApplicationListener {
  private static BaseAppListener        instance;

  public static Class<? extends Actor>  actorClassWildcard; // used as the Actor class signature for tween engine
  ...
  @Override public void create() {
    ...
    Tween.registerAccessor(actorClassWildcard,        new ActorAccessor());
    ...
    registerCommand("pref", args -> {
      if (args.length > 1) {
        putPref(args[0], (args.length > 2) ? arrToStrf("%s$| ", Arrays.copyOfRange(args, 1, args.length)) : args[2]);
      } else {
        log(pref(args[0]));
      }
    });
  ...
  public static BaseAppListener getInstance() {
    return instance;
  }
