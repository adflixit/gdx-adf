...
public abstract class BaseAndroidLauncher extends AndroidApplication implements XApi, GameHelperListener {
	private static ResultCallback<LoadPlayerScoreResult> playerScoreResultCallback = scoreResult -> setTmpScore(scoreResult.getScore().getRawScore());
	private static ResultCallback<LoadScoresResult> scoresResultCallback = scoreResult -> setTmpScore(scoreResult.getScores().get(0).getRawScore());
	...
	private void getTopScore(String id, int span, int collection) {
		Games.Leaderboards.loadTopScores(getApiClient(), id, span, collection, 1).setResultCallback(scoresResultCallback);
	}
	...
	@Override public long getPersonalBest(String id) {
		...
		(getApiClient(), id, TIME_SPAN_ALL_TIME, COLLECTION_PUBLIC).setResultCallback(playerScoreResultCallback);
		return tmpScore;
	}
	...
	@Override public void quit() {
		// minimizes the app, doesn't actually close it
		handler.post(() -> {
