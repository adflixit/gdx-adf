...
public class Confetti {
  ...
  void checkForReposition(int i) {
    if ((parX[i] > W + 20 || parX[i] < -20 || parY[i] > H) && confettiActive) {
      //66.67% of the flakes
      if (i % 5 > 0 || i % 2 == 0) {
        repositionParticle(i, randf(W), -10, floor(rand(10)) - 20);
      } else {
        if (sin(angle) > 0) {
          //Enter from the left
          repositionParticle(i, -20, randf(H), floor(rand(10)) - 20);
        } else {
          //Enter from the right
          repositionParticle(i, W + 20, randf(H), floor(rand(10)) - 20);
        }
      }
    }
  }

  void repositionParticle(int i, float xCoordinate, float yCoordinate, double tilt) {
    parX[i] = xCoordinate;
    parY[i] = yCoordinate;
    parTilt[i] = tilt;
  }
