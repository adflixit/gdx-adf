...
public final class Util {
   ...
   */
  private static float alignT(float pos, float len, int aln, int piv, int piv0, int piv1) {
    if (hasFlag(piv, piv0)) {
      if (hasFlag(aln, piv1)) {
        pos -= len;
   ...
   * @param piv1 highest pivot
   */
  private static float disalignT(float pos, float len, int aln, int piv, int piv0, int piv1) {
    if (hasFlag(piv, piv0)) {
      if (hasFlag(aln, piv1)) {
        pos += len;
      } else if (!hasFlag(aln, piv0) && !hasFlag(piv, piv1)) {
   ...
   * Sets alpha to 1.
   */
  public static Actor setupAlpha(Actor actor) {
    setAlpha(actor, 1);
    return actor;
  }
   ...
   * Sets alpha to 1.
   */
  public static void setupAlpha(Actor... actors) {
    for (Actor actor : actors) {
      setupAlpha(actor);
    }
  }
   ...
   * Sets alpha to 1.
   */
  public static Sprite setupAlpha(Sprite sprite) {
    sprite.setAlpha(1);
    return sprite;
  }
   ...
   * Sets alpha to 1.
   */
  public static void setupAlpha(Sprite... sprites) {
    for (Sprite sprite : sprites) {
      sprite.setAlpha(1);
    }
  }
